...
**Fix: „Tägliche Prüfzeit" wurde in UTC statt Server-Ortszeit interpretiert.** Live beobachtet (`daily_run_time=06:00`, Server-Zeitzone Europe/Berlin): der Versand ging nicht um 06:00 Uhr, sondern erst um 08:00 Uhr Ortszeit raus. Ursache: `lib/base.php` von Nextcloud ruft bei **jedem** Request `date_default_timezone_set('UTC')` auf — ein naives `new DateTimeImmutable('now')`/`'today'` liefert dadurch überall in der App-Laufzeit UTC, unabhängig von der tatsächlich konfigurierten Server-Zeitzone. Der Admin trägt „06:00" aber in der Erwartung „06:00 Ortszeit" ein, nicht UTC — bei Europe/Berlin im Sommer (CEST, UTC+2) ergab das eine Verschiebung um zwei volle Stunden; zusätzlich konnte in den 1-2 Stunden um lokal Mitternacht auch der Kalendertag für den Geburtstagsabgleich falsch bestimmt werden. Neue Klasse `lib/Service/ClockService.php` liest die tatsächlich konfigurierte Zeitzone direkt aus `ini_get('date.timezone')` (davon unberührt, da nur der PHP-*Laufzeit*-Default per Funktionsaufruf überschrieben wird, nicht der ini-Wert) und stellt `now()` (echte Uhrzeit, für den Zeitvergleich in `DailyReminderJob`/`CronTriggerController`) sowie `today()` (Mitternacht in Server-Ortszeit, für den Geburtstagsabgleich in `ReminderService`/`MembersApiController`/`DebugUpcoming`) bereit. Jede Stelle, die vorher naiv `new DateTimeImmutable('now'|'today')` konstruiert hat, nutzt jetzt `ClockService`.
...
**Ein Log-Eintrag je tatsächlich versendeter E-Mail, nicht je Mitglied-Treffer.** Ursprünglich loggte `ReminderService` genau einen Eintrag pro (Mitglied, Vorlaufzeit, Jahr) und schickte darunter an alle aufgelösten Empfänger-Adressen dieses einen Empfängers - bei einer Gruppen-/Mehrfach-Zuordnung verschwanden dadurch mehrere tatsächlich verschickte Mails hinter einem einzigen Log-Eintrag. Schlimmer: die Idempotenz-Prüfung war global auf (Mitglied, Typ, Vorlaufzeit, Jahr) skaliert, sodass beim ersten Empfänger, der eine Erinnerung für ein Match erhielt, dieses Match als „erledigt" markiert wurde - jeder **weitere** Empfänger, der für dieselbe Vorlaufzeit denselben Geburtstag abonniert hatte, wurde beim nächsten Schleifendurchlauf durch `alreadySent()` fälschlich übersprungen und bekam nie eine Mail. Migration `Version1040Date20260831000000` erweitert `oc_birthdayreminder_log` um `recipient_email` und den Unique-Index auf `(contact_uid, reminder_type, days_before, birthday_year, recipient_email)`; `alreadySent()`/`logSent()` sind jetzt pro Empfänger-Adresse skaliert, `sendRemindersForRecipient()` loggt entsprechend einen Eintrag je erfolgreich verschickter Mail statt einen je Match. Das behebt gleichzeitig den Mehrfach-Empfänger-Bug oben.

**„Kein Versand"-Markierung für leere Tage.** Damit im Log erkennbar ist, dass die automatische Prüfung an einem Tag tatsächlich lief und einfach nichts fällig war (statt dass der Job gar nicht gefeuert hat), schreibt `ReminderService::run()` über `ReminderLogMapper::logNoMailSent()` einen `ReminderLog::TYPE_NONE`-Eintrag, sobald ein kompletter Lauf (Erinnerungen + Glückwünsche zusammen) null tatsächlich verschickte Mails ergibt. Der Eintrag hat kein echtes Mitglied - `contact_uid` ist ein synthetischer Marker (`no-mail:YYYY-MM-DD`), damit er nie mit einer echten Mitglieds-ID kollidiert und trotzdem pro Kalendertag eindeutig bleibt (im Gegensatz zu echten Erinnerungs-Zeilen, die nur pro *Jahr* eindeutig sein müssen). Da `run()` dank `ScheduleGate`/`last_run_date` ohnehin höchstens einmal täglich durchläuft (egal ob über `DailyReminderJob` oder den externen Cron-Trigger), braucht die Markierung keinen eigenen Schutzmechanismus - ein seltener Doppelaufruf wird wie ein normaler `logSent()`-Race einfach über den Unique-Index abgefangen. Die manuellen „jetzt versenden"-Buttons lösen bewusst **keine** Markierung aus, da sie `runReminders()`/`runCongrats()` einzeln aufrufen, nicht das gemeinsame `run()`.

Bewusst **eine** Tabelle statt zweier getrennter Logs (Idempotenz vs. Anzeige): die zusätzliche `recipient_email`-Spalte erfüllt beide Zwecke gleichzeitig, ohne Buchhaltung doppelt zu führen.
...
**Fix: `DailyReminderJob` wurde außerhalb eines 4-Stunden-Fensters nie ausgeführt, wenn `maintenance_window_start` gesetzt ist.** Live auf einer Instanz beobachtet, deren echter CLI-Cron per SSH alle 5 Minuten lief, bei der aber trotzdem tagelang kein automatischer Versand stattfand: Nextclouds `CronService::runCli()` betrachtet den in Einstellungen → Verwaltung → Grundeinstellungen konfigurierbaren `maintenance_window_start` ("Zeitfenster für Wartungsarbeiten", z.B. `01` = 01:00 UTC) als bevorzugtes Low-Load-Fenster: **außerhalb** dieses ca. 4-Stunden-Fensters ruft es `IJobList::getNext($onlyTimeSensitive = true, ...)` auf, was per SQL nur noch Jobs mit `time_sensitive = TIME_SENSITIVE` liefert — ein als `TIME_INSENSITIVE` registrierter Job (der ursprüngliche, laut NC-Doku "rücksichtsvolle" Standardwert) wird dadurch den ganzen restlichen Tag über **komplett von der Auswahl ausgeschlossen**, unabhängig davon, wie zuverlässig `cron.php` selbst aufgerufen wird. `DailyReminderJob` ist daher jetzt bewusst `TIME_SENSITIVE` — der ganze Zweck dieser Erweiterung ist korrekter Versand zur eingestellten Zeit, nicht Rücksicht auf Systemlast.
